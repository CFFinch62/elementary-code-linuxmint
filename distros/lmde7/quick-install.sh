#!/bin/bash
# Quick installer for Elementary Code on LMDE 7
...
echo -e "${BOLD}${GREEN}║  for LMDE 7 (Debian Edition)              ║${NC}"
echo -e "${BOLD}${GREEN}╚════════════════════════════════════════════╝${NC}"
echo ""
...
if [ -f /etc/os-release ]; then
    ...
    if [ "$ID" != "linuxmint" ]; then
        echo -e "${YELLOW}Warning: This script is optimized for LMDE 7${NC}"
    ...
    # Warn if this is Ubuntu-based Linux Mint instead of LMDE
    if [ -n "$UBUNTU_CODENAME" ]; then
        echo -e "${YELLOW}Warning: Detected Ubuntu-based Linux Mint${NC}"
        echo "This script is for LMDE (Debian Edition)."
        echo "For Ubuntu-based Linux Mint, use: distros/linux-mint/quick-install.sh"
        echo ""
        read -p "Continue anyway? (y/N) " -n 1 -r
        echo
        if [[ ! $REPLY =~ ^[Yy]$ ]]; then
...
echo "  1. Install all required dependencies (Debian packages)"
...
echo ""
echo -e "${YELLOW}Note: LMDE uses Debian packages. Some libraries like Granite"
echo "may require manual installation if not in Debian repos.${NC}"
echo ""
